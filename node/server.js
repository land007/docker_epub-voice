...
const tts_server = process.env['TTS_SERVER'] || 'tts_server';
...
app.get('/eye/*', function(req, res) {
	// 在这里可以自定义你的路由分发
	let host = req.headers.host, ip = req.headers['x-forwarded-for']
...
app.get('/v1/Speech*', function(req, res) {
	proxy.on('proxyReq', function(proxyReq, req, res, options) {
		proxyReq.setHeader('Host', tts_server);
	});
	proxy.web(req, res, {
		target: 'http://' + tts_server + '/v1/Speech'
	});
});
